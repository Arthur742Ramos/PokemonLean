...
The formalization also future-proofs the framework for counterfactual analysis (e.g., ``what if a card is banned?'') and enables structural derivation of matchup implications from legal state transitions.
...
\paragraph{Algebraic sufficiency.}
A stronger, end-to-end theorem (\texttt{dark\_weakness\_sufficient\_for\_suboptimality}) proves that the Dark-type weakness \emph{alone} is algebraically sufficient to explain Dragapult's sub-50\% fitness.
...
\]
This is a machine-checked derivation from a single game rule (Psychic weak to Dark), through type assignments and empirical matchup data, to the paper's headline strategic conclusion.
The rules formalization is not disconnected infrastructure; it provides the structural explanation for the popularity paradox.

A companion module (\texttt{CausalChain.lean}) extends this bridge with 12 cross-module integration theorems that span the full infrastructure: type weakness amplified by Bo3 format, weakness-doubles-damage creating tempo advantages via energy economy, hypergeometric consistency feeding into Nash equilibrium, prize structure interacting with KO math and game length, and a summary theorem (\texttt{the\_complete\_story}) that conjoins 11 cross-module facts---from the single game rule \emph{Psychic is weak to Dark} through population shares, matchup losses, expected value, Bo3 amplification, variance reduction, and Swiss tournament exposure---into one machine-checked consistency check.
...
Importantly, uniqueness of this equilibrium is not proven: other Nash equilibria of the same game could in principle assign Dragapult positive weight (e.g., in a different equilibrium with a different column mix).
The verified result establishes that \emph{there exists} a Nash equilibrium excluding Dragapult, not that \emph{all} equilibria do so.
This distinction limits the prescriptive force of the Nash analysis: we can conclude that Dragapult exclusion is equilibrium-consistent, not that it is equilibrium-necessary.
