...
Our formal development spans approximately 30{,}000~lines across 80~Lean files and proves over 2{,}500 theorems without \texttt{sorry}, \texttt{admit}, or custom axioms.
...
The central result is a verified popularity-performance divergence (which we term the ``popularity paradox''): Dragapult Dusknoir is the most played deck (15.5\%) but has only 46.7\% expected win rate, while Grimmsnarl Froslass (5.1\% share) achieves 52.7\%.
...
We also solve the symmetrized constant-sum game $S_{ij} = (M_{ij} + 1000 - M_{ji})/2$, which yields a unique symmetric Nash equilibrium with seven-deck support preserving the same core decks, confirming robustness to the constant-sum approximation.
...
A 10{,}000-iteration equilibrium sensitivity analysis confirms the equilibrium's qualitative stability: the core support decks appear in $>$96\% of resampled equilibria, while Dragapult receives zero weight in 77.9\%.
These results demonstrate that formal methods can produce empirically grounded, machine-checked strategic insights in competitive game ecosystems; the specific metagame conclusions are illustrative of the methodology, which is the primary contribution.
\end{abstract}
...
Third, we derive machine-checked Nash-equilibrium statements over the full observed matrix and replicator-dynamics statements on representative 4-deck and 5-deck subgames, including complete best-response equilibrium certification.
...
\subsection{Game State and Turn Discipline}

The state model tracks per-player zones, turn ownership, and phase, sufficient to express legality constraints and prove conservation and progress properties.
...
Turn order is a finite phase machine (\texttt{draw}, \texttt{main}, \texttt{attack}, \texttt{betweenTurns}), making ``what can happen next'' decidable and enabling phase-safety lemmas (e.g., no attacks during draw phase).

\subsection{Type Effectiveness and Card Conservation}
...
For Professor's Research, we prove that discarding the hand and drawing seven preserves global card count:
...
Conservation theorems prevent subtle bookkeeping bugs from distorting probability estimates.
Deck legality is implemented as a computable checker linked to an inductive specification through a soundness-and-completeness biconditional (\texttt{checkDeckLegal\_iff}), guaranteeing that data ingestion is extensionally equivalent to the formal legality policy.

\subsection{Invariant Catalog}

The formal model maintains invariants including nonnegative zone sizes, deck-size preservation, bench-size bounds, uniqueness constraints for once-per-turn actions, deterministic transitions, prize-card accounting, and terminal-state exclusivity.
These prevent local rule inconsistencies from corrupting macro-level expected values.
Details of deck legality discussion, energy economy formalization, and the full invariant catalog are available in the supplementary artifact.
...
Table~\ref{tab:symmetric} reports the symmetric Nash equilibrium weights.
...
\begin{tabular}{lr}
\hline
\textbf{Archetype} & \textbf{Weight (\%)} \\
\hline
Grimmsnarl Froslass & 34.3 \\
...
As a preliminary predictive check, we compared our single-step replicator predictions against Trainer Hill trend data one day after the analysis window.
Two of three directional predictions were confirmed: Mega Absol showed upward trending (predicted: fastest growth) and Gardevoir showed upward trending (predicted: rising).
However, Grimmsnarl showed downward trending despite our model predicting growth---a secondary effect of Mega Absol's rise creating predation pressure on Grimmsnarl (Mega Absol beats Grimmsnarl 61.4\%).
This multi-step cascade is precisely the kind of indirect fitness effect that our single-step subgame model does not capture, reinforcing the caveat that subgame dynamics do not transfer directly to the full game.
A full iterated replicator simulation over the 14-deck system would be needed for accurate multi-step predictions.
