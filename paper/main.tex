...
In a large-field tournament, deck selection is a symmetric population game: each player independently chooses a deck and faces opponents drawn from the population mix. The natural solution concept is a symmetric Nash equilibrium. Our bimatrix formulation yields distinct row and column strategies because the empirical matrix is not exactly constant-sum ($M_{ij} + M_{ji} \neq 1000$ for some pairs due to tie weighting). In the symmetric population interpretation, the equilibrium recommendation is the average of the row and column strategies, which preserves the qualitative conclusion: the core support (Grimmsnarl, Mega Absol, Raging Bolt) is stable across both, and Dragapult receives near-zero weight in both. For players seeking a single mixed strategy, the column strategy (which includes Gholdengo at 3.7\% and excludes Alakazam) is marginally more conservative as it faces the row player's best response.
