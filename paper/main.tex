...
A machine-checked Nash equilibrium with six-deck support assigns Dragapult 0\% weight (uniqueness is not proven; other equilibria may differ); a symmetrized constant-sum formulation confirms this with seven-deck support.
...
Dragapult is not strictly dominated---it achieves the highest win rate of any deck against Charizard Noctowl (64.1\%)---so domination arguments cannot close this gap.
...
In principle, vertex enumeration algorithms (e.g., Avis--Fukuda pivoting~\cite{avis1992pivoting}) could enumerate all Nash equilibria of this 14$\times$14 bimatrix game; if every enumerated equilibrium excludes Dragapult, the existential result would strengthen to a universal one.
We leave this enumeration to future work, noting that the game's approximate constant-sum structure (deviations $<$3\% from $M_{ij}+M_{ji}=1000$) makes uniqueness plausible but not guaranteed.
...
\midrule
Rules \& legality        & $\sim$180  & Kernel   & \texttt{decide}, \texttt{simp} \\
Card effects             & $\sim$120  & Kernel   & \texttt{simp}, \texttt{omega} \\
Probability              & $\sim$90   & Kernel   & \texttt{decide}, \texttt{omega} \\
Step-size invariance     & 4          & Kernel   & \texttt{simp}, \texttt{ring}, \texttt{omega} \\
Infrastructure           & $\sim$1{,}860 & Kernel & mixed \\
\midrule
...
Other matrix computations& $\sim$127  & Compiler & \texttt{native\_decide} \\
\midrule
\textbf{Total}           & $\sim$2{,}500 & & \\
\bottomrule
\end{tabular}
\end{table}
...
The Lean-verified real Nash equilibrium has six-deck support per player and 0\% Dragapult weight (uniqueness not proven); single-step replicator dynamics on the full 14-deck matchup matrix indicate downward fitness pressure on Dragapult, upward pressure on Grimmsnarl, and strongest extinction pressure on Alakazam; and best-of-three math amplifies already-large matchup edges (67.3\%\,$\to$\,74.9\%).
