...
\begin{abstract}
We present a metagame analysis of the competitive Pok\'emon Trading Card Game, formally verified (modulo the \texttt{native\_decide} trust boundary discussed in Section~IX) using Lean~4 and real tournament data.
The artifact spans approximately 30{,}000 lines, 80 files, and 2{,}500 theorems---of which roughly 180 directly verify empirical claims---with no \texttt{sorry}, \texttt{admit}, or custom axioms.
...
First, we formalize the rules and legality substrate and establish a machine-checked bridge from type-effectiveness rules to empirical matchup outcomes (Section~\ref{subsec:typealignment}), demonstrating that the rules layer generates falsifiable predictions.
...
The formalization also future-proofs the framework for counterfactual analysis (e.g., ``what if a card is banned?'') and enables mechanical derivation of matchup implications from legal state transitions.
...
The formal connection runs: \emph{rules} (Psychic is weak to Dark) $\to$ \emph{type assignments} (Dragapult defends as Psychic, Grimmsnarl/Absol/Zoroark attack as Dark) $\to$ \emph{empirical confirmation} (Dark attackers hold positive win rates vs Dragapult) $\to$ \emph{weighted drag} (13.1\% of field imposes a population-level disadvantage).
...
A stronger, end-to-end theorem (\texttt{dark\_weakness\_sufficient\_for\_suboptimality}) proves that the Dark-type weakness \emph{alone} is sufficient to explain Dragapult's sub-50\% fitness.
...
\]
This is a machine-checked causal chain from a single game rule (Psychic weak to Dark), through type assignments and empirical matchup data, to the paper's headline strategic conclusion.
The rules formalization is not disconnected infrastructure; it provides the mechanistic explanation for the popularity paradox.

A companion module (\texttt{CausalChain.lean}) extends this bridge with 12 cross-module theorems that span the full infrastructure: type weakness amplified by Bo3 format, weakness-doubles-damage creating tempo advantages via energy economy, hypergeometric consistency feeding into Nash equilibrium, prize structure interacting with KO math and game length, and a ``grand causal chain'' (\texttt{the\_complete\_story}) that unifies all 11 conjuncts---from the single game rule \emph{Psychic is weak to Dark} through population shares, matchup losses, expected value, Bo3 amplification, variance reduction, and Swiss tournament exposure---into one machine-checked theorem.
...
\begin{table}[!t]
\centering
\caption{Lean-verified real Nash supports for row strategy \texttt{realNashRow} and column strategy \texttt{realNashCol}. Uniqueness is not proven.}
\label{tab:nash}
\scriptsize
\begin{tabular}{@{}rlrr@{}}
\toprule
...
\begin{table}[!htbp]
\caption{Symmetric Nash equilibrium on the constant-sum symmetrization $S_{ij} = (M_{ij} + 1000 - M_{ji})/2$. Uniqueness is not proven; this is the equilibrium found by the LP solver and verified in Lean.}
\label{tab:symmetric}
\centering
\footnotesize
\begin{tabular}{@{}lr@{}}
...
This does not logically preclude Dragapult from equilibria with different column strategies.
...
A general rational bridge (\texttt{rat\_replicator\_sign\_independent\_of\_dt} in the same file) extends this result to the \texttt{Lean.Rat} arithmetic used by the concrete replicator, proved symbolically without \texttt{native\_decide}. Concrete verification at $\mathrm{dt} = 1/10$, $1/100$, and $1$ (\texttt{StepSizeInvariance.lean}) confirms the identical 5-grower/9-shrinker partition in all cases.
Concrete verification for $\mathrm{dt} = 1/10$, $1/100$, and $1$ (\texttt{StepSizeInvariance.lean}) confirms the identical 5-grower/9-shrinker partition in all cases.
...
However, \texttt{decide} is \emph{structurally precluded}: our matrix computations use \texttt{Fin.foldl}, which is opaque to the Lean 4 kernel reducer and causes \texttt{decide} to fail immediately rather than time out.
...
This paper presents a metagame analysis pipeline for a real competitive TCG environment, verified modulo native code generation in Lean~4, demonstrating that formal methods can serve as a practical scientific instrument for competitive game ecosystems.
