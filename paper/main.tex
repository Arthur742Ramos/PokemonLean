...
A separate Python script performs exhaustive Nash equilibrium enumeration over all $2^{14}-1$ support subsets, confirming uniqueness of the symmetric NE and universal Dragapult exclusion (Section~\ref{sec:nash}); this enumeration is also external to the Lean artifact but independently verifiable.
...
\begin{table}[!htbp]
\caption{Lean-verified symmetric Nash equilibrium on the constant-sum symmetrization $S_{ij} = (M_{ij} + 1000 - M_{ji})/2$. Separately, exhaustive support enumeration on the raw symmetric game ($A = M$, $B = M^\top$) finds a unique 5-deck NE (Grimmsnarl 42.6\%, Raging Bolt 37.3\%, Gardevoir 8.9\%, Mega Absol 7.5\%, Charizard 3.7\%) that also excludes Dragapult; the support differences reflect the symmetrization.}
\label{tab:symmetric}
\centering
\footnotesize
\begin{tabular}{@{}lr@{}}
