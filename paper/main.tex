...
theorem mega_absol_dragapult_not_nash :
    ¬ NashEquilibrium realMetaGame
      megaAbsolDragapultCandidate megaAbsolDragapultCandidate := by
  native_decide
\end{lstlisting}

The support-concentrated profile in Table~\ref{tab:nash} is therefore reported as an LP-computed candidate equilibrium and verified only as a valid mixed strategy.
Theorems \texttt{mega\_absol\_dragapult\_row\_best\_response\_checks\_fail}, \texttt{mega\_absol\_dragapult\_col\_best\_response\_checks\_fail}, and \texttt{mega\_absol\_dragapult\_not\_nash} show this specific profile does not satisfy full best-response equilibrium conditions.
