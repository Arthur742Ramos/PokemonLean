...
The central result is a verified popularity paradox: Dragapult Dusknoir is the most played deck (15.5\%) but has only 46.7\% expected win rate against the field, while Grimmsnarl Froslass (5.1\% share) achieves the highest expected field win rate at 52.7\%.
The payoff matrix is derived from empirical tournament data; the rules formalization provides supporting infrastructure for deck legality, card conservation, and type effectiveness, establishing the formal context within which the strategic analysis operates.
We further report a fully verified Nash equilibrium (\texttt{real\_nash\_equilibrium\_verified}) with six-deck support per player; best-response conditions are machine-checked for all 14 pure strategies, the game value is approximately 47.97\%, and Dragapult receives 0\% equilibrium weight despite 15.5\% observed share.
We also prove replicator-dynamics pressure against Dragapult and Ceruledge on representative observed-data subgames (4-deck and 5-deck slices of the full 14-archetype matrix), and quantify best-of-three amplification effects such as 67.3\%\,$\to$\,74.9\% for Raging Bolt Ogerpon versus Mega Absol Box.
...
(iii) fully verified Nash-equilibrium computation (including all 14 pure-strategy best-response checks) and replicator-dynamics statements on real-data representative subgames, and
...
Third, we derive fully verified Nash-equilibrium statements over the full observed matrix and replicator-dynamics statements over real-data representative subgames, including complete best-response equilibrium certification.
...
\begin{lstlisting}
-- PokemonLean/Core/Types.lean:206
...
\begin{lstlisting}
-- PokemonLean/Core/Types.lean:181
...
\begin{lstlisting}
-- PokemonLean/TypeEffectiveness.lean:138
theorem TRIANGLE :
    ∃ A B C : PType,
...
\begin{lstlisting}
-- PokemonLean/CardEffects.lean:149
...
\begin{lstlisting}
-- PokemonLean/DeckLegality.lean:107
...
\begin{lstlisting}
-- PokemonLean/DeckConsistency.lean:49
...
\begin{lstlisting}
-- PokemonLean/EnergyEconomy.lean:37
...
\begin{lstlisting}
-- PokemonLean/RealMetagame.lean:398
...
\begin{table*}[!t]
\centering
\caption{Expected win rate on the modeled top-14 subset (69.5\% of field; ``Other'' excluded). Tiers are assigned by a combination of expected field win rate and matchup profile breadth: S~($\geq$52\%), A~(50--52\% or strong matchup coverage), B~(competitive but sub-50\% or narrow coverage), C~(below-average expected win rate with unfavorable breadth).}
\label{tab:expected}
\begin{tabular}{lccc}
\toprule
...
We therefore treat this as a modeling limitation: the analysis below targets a minimax benchmark for a single match, not a full Swiss-utility optimum.

\begin{lstlisting}
-- PokemonLean/NashEquilibrium.lean:320
theorem real_nash_row_best_response_checks :
    ∀ i : Fin 14, rowPurePayoff realMetaGame14 i realNashCol ≤ realNashValue := by
  native_decide

-- PokemonLean/NashEquilibrium.lean:324
theorem real_nash_col_best_response_checks :
    ∀ j : Fin 14, realNashValue ≤ colPurePayoff realMetaGame14 realNashRow j := by
  native_decide

-- PokemonLean/NashEquilibrium.lean:328
...
The theorem \texttt{real\_nash\_equilibrium\_verified} is backed by best-response checks quantified over all 14 pure strategies for both players, so the equilibrium claim is now fully machine-checked rather than merely mixed-strategy-valid.
The game value is \texttt{realNashValue} $= \frac{162188991282520}{338129962783} \approx 479.665$ (47.97\%).
Win rates are encoded on a 0--1000 scale, so a game value of 479.67 corresponds to a 47.97\% win probability.
...
Indices in Table~\ref{tab:nash} use the zero-based \texttt{Deck.toFin} mapping from \texttt{PokemonLean/RealMetagame.lean}:
...
This means the payoff matrix does not satisfy the strict zero-sum condition $M_{ij} = 1000 - M_{ji}$, which is the classical prerequisite for identical row and column equilibrium strategies.
In a game with such ``near-zero-sum'' but not exactly zero-sum structure, distinct row and column supports are mathematically expected rather than anomalous.
...
Lean theorems \texttt{dragapult\_share\_decreases}, \texttt{ceruledge\_share\_decreases}, and \texttt{mega\_absol\_grows\_fastest} (\texttt{PokemonLean/EvolutionaryDynamics.lean}) provide concrete machine-checked directional updates for real-data metagame subgames.
\begin{lstlisting}
-- PokemonLean/EvolutionaryDynamics.lean:463
...
-- PokemonLean/EvolutionaryDynamics.lean:665
...
-- PokemonLean/EvolutionaryDynamics.lean:523
...
\begin{lstlisting}
-- PokemonLean/TournamentStrategy.lean:31
def bo3WinProb (p : Rat) : Rat :=
  p * p * (3 - 2 * p)

-- PokemonLean/TournamentStrategy.lean:41
def favoredRates : List Rat :=
  [11/20, 12/20, 13/20, 14/20, 15/20, 16/20, 17/20, 18/20, 19/20]

-- PokemonLean/TournamentStrategy.lean:46
...
This is standard practice in the Lean and Mathlib communities for large finite decision problems where \texttt{decide} would require prohibitive compilation time or memory.
However, \texttt{native\_decide} represents a different trust level than kernel-checked proofs: correctness depends on the native code compiler in addition to the kernel.
All uses of \texttt{native\_decide} in this development could in principle be replaced by \texttt{decide} at the cost of substantially longer build times, and we have verified this for a representative subset of the claims.
...
Grimmsnarl's top expected value, the fully verified six-deck Nash equilibrium (including zero Dragapult support), Raging Bolt's 67.3\% counter edge, and Bo3 amplification values.
...
\begin{lstlisting}
-- PokemonLean/Robustness.lean:54
...
The Lean-verified real Nash equilibrium has six-deck support per player and 0\% Dragapult weight; on representative observed-data subgames (not the full 14-deck matrix), replicator dynamics predict Dragapult decline and Ceruledge extinction pressure; and best-of-three math amplifies already-large matchup edges (67.3\%\,$\to$\,74.9\%).
