...
(iii) Nash equilibrium and replicator dynamics statements, and
...
Third, we derive and verify Nash and replicator dynamics statements over the observed matrix.
...
We use Wilson intervals of the form
\[
...
For smaller matchups (e.g., Ceruledge vs N's Zoroark: $\sim$100 games), intervals widen to about $\pm 9$ points.
...
We emphasize that these behavioral hypotheses are plausible explanations consistent with established theory, not measured causal mechanisms.
Distinguishing anchoring from herding from information cascades in deck-choice data would require player-level panel data with temporal resolution, which is beyond the scope of this analysis.
Our contribution is the formal proof that the aggregate outcome is suboptimal under the stated payoff model, regardless of its behavioral cause.
...
This divergence motivates equilibrium and dynamic analysis in Section~\ref{sec:nash}.
...
Existence of mixed Nash equilibrium follows from finite minimax and Nash's equilibrium existence framework~\cite{vonneumann1928theorie,nash1950equilibrium}.
...
Section~\ref{sec:methodology} then details the reproducibility workflow that underpins these recommendations.
...
We provide formal worst-case bounds.
Even under the extreme assumption that Dragapult achieves 100\% win rate against all unmodeled archetypes while Grimmsnarl achieves 0\%, the adjusted expected win rates are XX.X\% and YY.Y\%, respectively.
The popularity paradox reversal requires Dragapult to simultaneously achieve at least X\% against the unmodeled field while Grimmsnarl drops below Y\%, a scenario unsupported by available evidence.
These bounds are machine-checked (Theorem \texttt{robustness\_bound} in \texttt{Robustness.lean}).

\begin{lstlisting}
theorem robustness_bound :
    adjustedEV DragapultDusknoir 1 <=
      adjustedEV GrimmsnarlFroslass 0 := by
  native_decide
\end{lstlisting}
